...
public class find_table {
    public static void encontrar_tabla(String inputFilePath) throws IOException {
        // Cargar archivo de Excel
        FileInputStream fileInputStream = new FileInputStream(inputFilePath);
        try (XSSFWorkbook workbook = new XSSFWorkbook(fileInputStream)) {
            XSSFSheet originalSheet = workbook.getSheetAt(1); 

            // Crear una nueva hoja para copiar los datos
            XSSFSheet newSheet = workbook.createSheet("Hoja2");
            ...
            workbook.setSheetName(workbook.getSheetIndex(newSheet), "Hoja1");
